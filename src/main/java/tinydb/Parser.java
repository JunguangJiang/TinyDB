...
public class Parser {
    ...
    public static Predicate.Op getOp(String s) throws tinydb.ParsingException {
        if (s.equals("="))
            ...
            if (isJoin) { // join node
                ...
                if (!op2const) { // right op is a nested query
                    try {
                        ...
                        throw new tinydb.ParsingException("Invalid subquery "
                                + ops.elementAt(1));
    ...
    public LogicalPlan parseQueryLogicalPlan(TransactionId tid, ZQuery q)
            throws IOException, Zql.ParseException, tinydb.ParsingException {
        @SuppressWarnings("unchecked")
        ...
        for (int i = 0; i < from.size(); i++) {
            ...
            try {
                ...
                e.printStackTrace();
                throw new tinydb.ParsingException("Table "
            ...
            if (si.getAggregate() != null) {
                ...
                if (groupByField != null
                        && !(groupByField.equals(si.getTable() + "."
                                ...
                                .getColumn()))) {
                    throw new tinydb.ParsingException("Non-aggregate field "
                            + si.getColumn()
        ...
        if (groupByField != null && aggFun == null) {
            throw new tinydb.ParsingException("GROUP BY without aggregation.");
        }
    ...
    public Query handleQueryStatement(ZQuery s, TransactionId tId)
            ...
            tinydb.ParsingException, Zql.ParseException {
        ...
        if (physicalPlan != null) {
            Class<?> c;
            try {
                c = Class.forName("tinydb.OperatorCardinality");
                ...
                c = Class.forName("tinydb.QueryPlanVisualizer");
                m = c.getMethod(
    ...
    public Query handleInsertStatement(ZInsert s, TransactionId tId)
        ...
        try {
            ...
            throw new tinydb.ParsingException("Unknown table : "
                    + s.getTable());
        }
            ...
            for (ZExp e : values) {
                ...
                if (zc.getType() == ZConstant.NUMBER) {
                    if (td.getFieldType(i) != Type.INT_TYPE) {
                        throw new tinydb.ParsingException("Value "
                                + zc.getValue()
    ...
    public void handleTransactStatement(ZTransactStmt s)
        ...
        if (s.getStmtType().equals("COMMIT")) {
            ...
            throw new tinydb.ParsingException("Unsupported operation");
        }
    }
    ...
    public LogicalPlan generateLogicalPlan(TransactionId tid, String s)
        ...
        try {
            ...
            throw new tinydb.ParsingException(e);
        }
        ...
        try {
            ...
            if (s instanceof ZTransactStmt)
                ...
                try {
                    ...
                    if (a instanceof tinydb.ParsingException
            ...
            e.printStackTrace();
        } catch (tinydb.ParsingException e) {
            System.out
